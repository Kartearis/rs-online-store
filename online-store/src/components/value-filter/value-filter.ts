...
class ValueFilter extends HTMLElement {
    ...
    createOption(option: Option): void {
        ...
        optionCheckbox.addEventListener('change', () => this.optionSelected());
    ...
    optionSelected(): void {
        const options: NodeListOf<Element> = assertDefined(this.#optionContainer).querySelectorAll(
            '.value-filter__option:checked'
        );
        const selected: FilterState = Array.from(options).map(
            (element) => (element as HTMLInputElement).dataset['value']
        );
    ...
    set label(data: string | undefined) {
        if (data) assertDefined(this.#innerData).label = data;
    }
}
